function GrasppeJive(parameters) {GrasppeJive.prototype.initialize.apply(this, arguments)};
GrasppeJive.prototype = {
  title: 'GrasppeJive', version: 0.1,
  description: 'Sandbox for computational math operations.',
  definitions: [], _definitions: [], _stack: [],
  _sandbox: null, _i: -1, _fn: null, _id: null, _val: null, _fx: null, _v: null, _t: null, _c: null,
  _function: {Math: ['round', 'min', 'max', 'sqrt', 'pow', 'PI', 'cos', 'sin', 'atan', 'atan2']},
  ...
  initialize: function(parameters, definitions, callback){
    ...
    ['title', 'version', 'description'].forEach(function (property) { this[property] = prototype[property]; });
    ...
    Object.assign(this, { _stack: [], rows: [], logs: [], errors: []});
    
    this.setParameters(parameters);
    return this;
  },
  getParameters: function(key) {
    if (key) return this.parameters[key]; else return this.parameters;
  },
  setParameters: function(key, value) {
    if (typeof value === 'undefined' && typeof (key) !== 'object') delete this.parameters[key];
    else if (typeof (key) === 'object') Object.keys(key).forEach(function(thisKey) {this.parameters[thisKey] = key[thisKey];}.bind(this));
  ...
  run: function(_scenario, _stack){
    ...
    for(var _i = 0; _i < this._definitions.length; _i++) {
      ...
      try {   
        var _val = NaN, _id = this._id, _fn = this._fn.fn;
  ...
  callback: function(_fn) {this.rows.push([_fn.id, _fn.value, _fn.unit]);},
  error: function(message) {return this.errors.push(message) && false;},
  log: function(message) {return this.logs.push(message) || true;},
};