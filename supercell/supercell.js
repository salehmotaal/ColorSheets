...
(function (window, grasppe, undefined) {
    ...
    grasppe.require(['colorsheets'], function () {
        ...
        grasppe.ColorSheetsApp.SupercellDemo = grasppe.ColorSheetsApp.createModel('SupercellDemo', {
            title: ('Supercell Demo'),
            path: 'supercell', panels: {
                stage: {
            ...
            directive: 'supercell-demo-sheet', directives: {
                ...
                supercellDemoSheet: function supercellDemoSheet() {
                    return {
                        ...
                        template: ('<color-sheets-sheet></color-sheets-sheet><color-sheets-documentation-dialog/>'),
                    }
                },
                ...
                supercellSheetResults: function () { // grasppe.Libre.Directive.define('supercellSheetResults',
                    return {
                        ...
                        // ng-if="row.value || $sheet[row.variable]"
                    }
                },
                // ),
        ...
        grasppe.ColorSheetsApp.SupercellDemoController = grasppe.ColorSheetsApp.createController(function SupercellDemoController($scope, element) {
            ...
            Object.assign($scope.$sheet, {
                helper: controller, calculations: {},
                stack: {},
                canvas: {},
                destructer() {
            ...
            get calculations() {
                return this.$scope && this.$sheet.calculations || {};
            }, set calculations(calculations) {
                if (this.$scope) this.$sheet.calculations = Object.assign(this.$sheet.calculations || {}, calculations);
            }, get stack() {
                return this.$scope && this.$sheet.stack;
            }, set stack(stack) {
                if (this.$scope) this.$sheet.stack = Object.assign(this.$sheet.stack || {}, stack);
            }, get scenarios() {
                return grasppe.ColorSheetsApp.SupercellDemo.scenarios
            },
            getParameter(parameter) {
                ...
                var values = this.calculations,
                    options = Object.assign({}, grasppe.ColorSheetsApp.SupercellDemo.options, this.$sheet.options),
                ...
                try {
                    ...
                    for (var i = 0; i < cells; i++) for (var j = 0; j < cells; j++) { // if (((i % 2) + (j % 2) !== 1)) {
                        ...
                        for (var q = 0; q < imageData.height; q++) for (var p = 0; p < imageData.width; p++) {
                            var channel = 3,
                                pixelValue = rawData[(imageData.width * q + p) * 4 + channel],
                                surroundValue = (0 + rawData[(imageData.width * q - 1 + p - 1) * 4 + channel] + rawData[(imageData.width * q + 0 + p - 1) * 4 + channel] + rawData[(imageData.width * q + 1 + p - 1) * 4 + channel] + rawData[(imageData.width * q - 1 + p + 0) * 4 + channel] + rawData[(imageData.width * q + 0 + p + 0) * 4 + channel] + rawData[(imageData.width * q + 1 + p + 0) * 4 + channel] + rawData[(imageData.width * q - 1 + p + 1) * 4 + channel] + rawData[(imageData.width * q + 0 + p + 1) * 4 + channel] + rawData[(imageData.width * q + 1 + p + 1) * 4 + channel]) / 9;
                            // console.log(pixelValue, surroundValue);
                            if ((pixelValue > surroundValue && surroundValue > 15) || pixelValue > 127) { // 127) { // Math.round(256/9*2)-1
                                var xPixel = Math.floor((xMin + x1 + p + 0.001 - offset)),
                                    yPixel = Math.floor((yMin + y1 + q + 0.001 - offset));
